...
public class MainActivity extends Activity implements OnClickListener {
    ...
    private void updateAccel() {
        ...
        acc_x.setText(String.format("X%6.2f", accelBuffer[0]));
        acc_y.setText(String.format("Y%6.2f", accelBuffer[1]));
        acc_z.setText(String.format("Z%6.2f", accelBuffer[2]));
        if(accelBuffer[1] < -0.20d) {
            new NetworkGuy().execute("a");
            Log.d("Hi", "I AM TURNING");
        }
        else if(accelBuffer[1] > 0.20d)
            new NetworkGuy().execute("d");
        else
            if (accelBuffer[2] > 0.95)
                new NetworkGuy().execute("s");
            else if (accelBuffer[2] < 0.85)
                new NetworkGuy().execute("w");
            else
                new NetworkGuy().execute(" ");
